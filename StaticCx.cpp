//---------------------------------------------------------------------------
// CStatiCx Copyright (C) 2013 Crystal Dew World All rights reserved.
//---------------------------------------------------------------------------
...
IMPLEMENT_DYNAMIC(CStaticCx, CStatic)

BEGIN_MESSAGE_MAP(CStaticCx, CStatic)
	//{{AFX_MSG_MAP(CStaticCx)
	ON_WM_MOUSEMOVE()
	ON_WM_MOUSEHOVER()
	ON_WM_MOUSELEAVE()
	ON_WM_KILLFOCUS()
...
// CStaticCx メッセージ ハンドラ
//------------------------------------------------
// ツールチップ関連
//------------------------------------------------

// ツールチップを初期化する。
void CStaticCx::InitToolTip()
{
	// ツールチップは未初期化？
...
// コントロールに渡る前のメッセージを処理。
BOOL CStaticCx::PreTranslateMessage(MSG* pMsg) 
{
	// ツールチップを初期化する。
	InitToolTip();
...
// ツールチップのメッセージを設定する。
void CStaticCx::SetToolTipText(LPCTSTR pText)
{
	// ツールチップを初期化する。
	InitToolTip();
...
// ツールチップの有効無効を切り替えます。
void CStaticCx::SetToolTipActivate(BOOL bActivate)
{
	// ツール ヒント コントロールに登録されているツールの数は無効？
...
void CStaticCx::SetToolTipWindowText(LPCTSTR pText)
{
...
// コントロールのオーナドロー
void CStaticCx::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	// 描画のＣＤＣ
	...
	if(IsWindowEnabled())
	{
		...
		{
			// 押されたボタンの描画
			DrawControl(drawDC, lpDrawItemStruct, m_CtrlBitmap, m_BgBitmap, 0);//2);
		}
		else
		{
			// ホバー状態？
...
void CStaticCx::DrawString(CDC *drawDC, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	// テキストを得る。
	CString title;
	GetWindowText(title);
...
// コントロールの描画
void CStaticCx::DrawControl(CDC *drawDC, LPDRAWITEMSTRUCT lpDrawItemStruct, CBitmap &CtrlBitmap, CBitmap &BgBitmap, int no)
{
#if CHECK_DRAWTIME
	// 時間計測
...
// リソースからビットマップを割り当てる。
BOOL CStaticCx::LoadBitmap(UINT nIDResource, LPCTSTR pResourceType)
{
...
// ファイルからビットマップ割り当てる。
BOOL CStaticCx::LoadBitmap(LPCTSTR pFname)
{
	if (m_bHighContrast) { return FALSE; }
...
// HBITMAPからビットマップを割り当てる。
BOOL CStaticCx::LoadBitmap(HBITMAP hbm)
{
	if (m_bHighContrast) { return FALSE; }
...
// ビットマップデータのセットアップ
BOOL CStaticCx::SetBitmap(CBitmap &cbitmap)
{
	if (m_bHighContrast) { return FALSE; }
...
// コントロールサイズを得る。
CSize CStaticCx::GetSize(void)
{
	return m_CtrlSize;
}

// 背景を取り込む。
void CStaticCx::LoadCtrlBg(CDC* drawDC)
{
...
void CStaticCx::SetBgReload(void)
{
	// 再ロードを指示。
	m_bBgBitmapInit = FALSE;
	m_bBgLoad = FALSE;
...
// マウスの移動
void CStaticCx::OnMouseMove(UINT nFlags, CPoint point) 
{
	// トラッキングしていない？
	if(! m_bTrackingNow)
	{
...
// マウスが上にあるイベント。
void CStaticCx::OnMouseHover(UINT nFlags, CPoint point)
{
...
BOOL CStaticCx::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	if(m_bHandCursor)
	{
		// ハンドカーソルに変更
...
void CStaticCx::SetHandCursor(BOOL bHandCuror)
{
	m_bHandCursor = bHandCuror;
}

void CStaticCx::SetDrawFrame(BOOL bDrawFrame)
{
	if (bDrawFrame)
	{
		// スタティックエッジを有効
...
void CStaticCx::SetMargin(int top, int left, int bottom, int right, double zoomRatio)
{
...
void CStaticCx::SetAlpha(BYTE alpha)
{
	m_Alpha = alpha;
}